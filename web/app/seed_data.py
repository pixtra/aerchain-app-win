...
def _make_supplier_products(skus: list[Sku], line_sku_ids: set[str]) -> list[SupplierProduct]:
    """249 rows; every supplier maps every RFx-line SKU, plus extras for the remainder.

    SKU-049/050 (L29/L30) are mapped across all suppliers but receive no quotes.
    """
    products = []
    idx = 1
    ...
    for si, sup in enumerate(_SUPPLIERS):
        ...
        for sku in bucket:
            ...
            products.append(SupplierProduct(
                ...
                supplier_description=f"{sku.specification} – {sup[2]} grade",
...
# ---------------------------------------------------------------- RFx + 30 lines
def _make_rfx_and_lines(skus: list[Sku]) -> tuple[Rfx, list[RfxLine]]:
    rfx = Rfx(
        rfx_id=RFX,
        rfx_name="Corrugated Packaging & Shipping Materials FY26",
...
def _make_offers(lines, products, skus, suppliers) -> tuple[list[SupplierOffer], list[DiscountTier], list[OfferEvidence]]:
...
def build() -> dict:
    ...
    rfx, lines = _make_rfx_and_lines(skus)
    line_sku_ids = {l.sku_id for l in lines}
    products = _make_supplier_products(skus, line_sku_ids)
    fx, policy = _make_fx_and_policy()
    questions = _make_questionnaire()
    rfx_suppliers = _make_rfx_suppliers()
    offers, tiers, evidence = _make_offers(lines, products, skus, suppliers)

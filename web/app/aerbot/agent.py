...
SYSTEM = """You are AerBot, Aerchain's procurement AerBot for a "Kill-the-Spreadsheet" RFx evaluation
...
_CHITCHAT_RE = re.compile(
    r"^(hi+|hii+|hello+|hey+|yo|good\s*(morning|afternoon|evening|day)|"
    r"thanks?|thank\s*you|thx|bye+|goodbye|see\s*you(\s*later)?|"
    r"who\s*are\s*you|what'?s\s*your\s*name|what\s*can\s*you\s*do|"
    r"help(\s*me)?|how\s*are\s*you)\W*$", re.I)
...
TOOLS = [
    ...
    _tool("get_full_brief",
          ...
          "conditions. MANDATORY for any question about all lines, every "
          "line, the full RFx, or bulk analysis — never call per-line tools "
          "in a loop.",
          {"type": "object", "properties": {}, "additionalProperties": False}),
...
def run(question: str, chat_fn=None, steps: int = MAX_STEPS,
    ...
    from .llm import client_chat
    chat_fn = chat_fn or client_chat
    if _is_chitchat(question) and not history:
    ...
    raise RuntimeError(
        "agent could not produce a grounded answer in the tool-call budget")
...
def status() -> dict:
    ...
    try:
        chain = resolve_chain()
        if not chain:
            raise ModelNotConfigured("empty chain")
        primary, cfg = chain[0]

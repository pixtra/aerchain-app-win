  ...
  try {
    ...
    placeholder.outerHTML = slow
      ? `<div class="msg answer"><div class="who">${BOT}</div><div class="t">Still thinking — the connection gave up first</div>
         ...
         (public links time out at ~100s; hard questions on the local model can take 1–3 min).
         Nothing is broken — try one of these:</div>
         <div class="assump">Ask a smaller question (one line or supplier) · Use the bulk brief suggestion for whole-RFx analysis · Switch to <strong>Groq cloud in Settings</strong> for answers in seconds · Your chat so far is saved</div>
...
async function viewSettings() {
  const st = await api("/settings/provider").catch(() => ({ mode: "offline", chain: [] }));
  const modes = [
    ["offline", "Offline (local model)", "Private. Data never leaves this machine. ~25s per answer.", ""],
    ["groq", "Groq cloud", "Fast + stronger NL. Needs a free key (console.groq.com/keys). Falls back to local automatically.", "Recommended"],
    ["keyless", "Keyless demo cloud", "Zero setup, but shared/free: variable speed, no SLA, unclear data retention. Dummy data only.", ""],
  ];
  VIEW.innerHTML = `
  <h2 class="view-title">Settings — AI provider</h2>
  <div class="card" style="max-width:720px"><h3>Mode</h3>
    ${modes.map(([v, t, d, tag]) => `
      <label style="display:block;margin:10px 0">
        <input type="radio" name="pmode" value="${v}" ${st.mode === v ? "checked" : ""} />
        <strong>${t}</strong>${tag ? ` <span class="badge good">${tag}</span>` : ""}<br/><span class="note">${d}</span>
      </label>`).join("")}
    <div id="groq-key-row" style="margin:10px 0;${st.mode === "groq" ? "" : "display:none"}">
      <label class="note">Groq API key ${st.has_groq_key ? "(saved — leave blank to keep it)" : "(console.groq.com/keys, free)"}</label><br/>
      <input id="set-key" type="password" placeholder="gsk-…" style="width:320px" autocomplete="off" />
    </div>
    <div class="filterbar">
      <label class="note">OCR languages</label>
      <input id="set-ocr" placeholder="eng" value="${esc(st.ocr_lang || "eng")}" title="Tesseract languages, e.g. eng or eng+hin (needs the data pack)" style="width:120px" />
    </div>
    <div class="filterbar">
      <button class="btn primary" id="btn-save-mode">Save</button>
      <button class="btn ghost" id="btn-test-mode">Test (~5s)</button>
    </div>
    <div id="set-out" class="note"></div>
    <div class="assump">Active chain: ${(st.chain || []).map(esc).join(" → ") || "none (LLM offline)"}
      ${st.serving ? `<br/>last answer served by ${esc(st.serving)}` : ""}</div>
  </div>
  <div class="card" style="max-width:720px;margin-top:16px"><h3>Local model ${info("The offline brain. If the required model is missing, install it here (multi-GB download) — cloud modes fall back to it automatically.")}</h3>
    <div id="model-list" class="note">checking…</div>
    <div id="model-pull" style="margin-top:8px"></div>
  </div>
  <div class="card" style="max-width:720px;margin-top:16px"><h3>Demo data ${info("Live counts straight from the database. Factory reset rebuilds the golden dataset and wipes uploads — the panic button before a demo.")}</h3>
    <div id="data-stats" class="note">loading…</div>
    <div class="filterbar" style="margin-top:10px">
      <button class="btn ghost" id="btn-factory">Factory reset</button>
      <span class="note">golden rebuild + wipe uploads — pristine demo state</span>
    </div>
  </div>`;
  VIEW.querySelectorAll('input[name="pmode"]').forEach((r) => (r.onchange = () => {
    $("groq-key-row").style.display = r.value === "groq" ? "" : "none";
  }));
  $("btn-save-mode").onclick = async () => {
    const mode = VIEW.querySelector('input[name="pmode"]:checked').value;
    const body = { mode };
    if (mode === "groq" && $("set-key").value.trim()) body.groq_key = $("set-key").value.trim();
    if ($("set-ocr").value.trim()) body.ocr_lang = $("set-ocr").value.trim();
    $("set-out").textContent = "saving…";
    try {
      const s = await api("/settings/provider", { method: "POST",
        headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      $("set-out").textContent = `saved — chain: ${s.chain.join(" → ")}`;
      $("set-key").value = "";
      toast("Provider mode saved", true);
    } catch (e) { $("set-out").textContent = e.message; }
  };
  $("btn-test-mode").onclick = async () => {
    $("set-out").textContent = "testing live call…";
    try {
      const t = await api("/settings/provider/test", { method: "POST" });
      $("set-out").textContent = t.ok
        ? `OK in ${t.ms}ms — served by ${t.serving}${t.fell_back ? " (fallback engaged)" : ""}`
        : `unexpected reply from ${t.serving}`;
    } catch (e) { $("set-out").textContent = e.message; }
  };
  const loadModels = async () => {
    let inv;
    try { inv = await api("/settings/models"); }
    catch (e) { $("model-list").textContent = "model inventory unavailable"; return; }
    if (!inv.reachable) {
      $("model-list").innerHTML = `Ollama isn't running — offline mode needs it.<br/>`
        + `<span class="note">Start it (<span class="mono">ollama serve</span>), install it from `
        + `<span class="mono">https://ollama.com/download</span>, or switch to Groq/keyless above — no install needed.</span>`;
      return;
    }
    $("model-list").innerHTML =
      (inv.models.length
        ? inv.models.map((m) => `<div>• <span class="mono">${esc(m.name)}</span> <span class="note">${m.size_gb} GB</span></div>`).join("")
        : `<div class="empty">No models downloaded yet.</div>`)
      + `<div class="note" style="margin-top:6px">stored in <span class="mono">${esc(inv.models_dir || "")}</span></div>`
      + `<div style="margin-top:8px">Required local model: <span class="mono">${esc(inv.required)}</span> `
      + (inv.installed ? badge("good", "INSTALLED")
        : `${badge("warn", "MISSING")} <button class="btn primary" id="btn-pull">Install (~5 GB)</button>`) + `</div>`;
    const btn = $("btn-pull");
    if (btn) btn.onclick = async () => {
      btn.disabled = true;
      btn.textContent = "starting…";
      try { await api("/settings/models/pull", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" }); }
      catch (e) { $("model-pull").textContent = e.message; btn.disabled = false; return; }
      const poll = setInterval(async () => {
        try {
          const s = await api("/settings/models/pull-status");
          const tail = (s.log || []).slice(-3).join(" · ");
          $("model-pull").innerHTML = `<div class="note">${esc(tail)}</div>`
            + `<div class="progress" style="margin-top:6px"><div style="width:${s.done ? 100 : 15}%"></div></div>`;
          if (s.done) {
            clearInterval(poll);
            $("model-pull").innerHTML = s.error
              ? `<div class="empty">Install failed: ${esc(s.error)}</div>`
              : `<div class="assump">Installed — offline mode is ready.</div>`;
            loadModels();
          }
        } catch (_) {}
      }, 2000);
  ...
  try {
    ...
    $("data-stats").innerHTML = `<div class="grid kpi-row">
      <div class="kpi"><div class="v">${s.offers}</div><div class="l">Decision offers</div></div>
      <div class="kpi good"><div class="v">${s.eligible}</div><div class="l">Eligible</div></div>
      <div class="kpi"><div class="v">${s.lines_awarded}</div><div class="l">Lines awarded</div></div>
      <div class="kpi"><div class="v">${s.uploads}</div><div class="l">Uploads (${s.upload_files_on_disk} files)</div></div>
      <div class="kpi"><div class="v">${s.conditions}</div><div class="l">Active conditions</div></div>
    </div>`;
  } catch (_) { $("data-stats").textContent = "stats unavailable"; }
  ...
  $("btn-factory").onclick = async () => {
    ...
    try {
      ...
      toast(`Pristine state restored (${r.uploads_removed} uploads wiped)`, true);
      viewSettings();
    } catch (e) {
...
$("btn-reset").onclick = async () => {
  ...
  toast("Dataset re-seeded. Switching to Overview.", true);

...
def _ocr_pdf_pages(path: str) -> list[str]:
    """Render PDF pages and OCR each one. Prefers PyMuPDF (works on Windows),
    falls back to pdftoppm (Linux/macOS poppler). Returns text lines."""
    import tempfile
    from PIL import Image
    import pytesseract
    out = []
    with tempfile.TemporaryDirectory(prefix="ktq_pdf_") as tmp:
        pngs = _render_pdf_pages(path, tmp)
        for fn in pngs:
...
def _render_pdf_pages(path: str, tmp: str) -> list[str]:
    """Return PNG file paths, one per page. PyMuPDF first (cross-platform)."""
    try:
        import fitz  # PyMuPDF
        doc = fitz.open(path)
        out = []
        for i, page in enumerate(doc):
            fn = os.path.join(tmp, f"pg-{i + 1:03d}.png")
            page.get_pixmap(dpi=200).save(fn)
            out.append(fn)
        return out
    except ImportError:
        pass
...
def _ocr_text(path: str) -> str:
    ...
    # Windows installs Tesseract outside PATH — allow an explicit location
    # via TESSERACT_CMD (e.g. C:\Program Files\Tesseract-OCR\tesseract.exe)

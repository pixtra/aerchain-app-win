...
def settings_provider_set(req: ProviderSettingsRequest):
    ...
    try:
        return set_provider_settings(req.mode, req.groq_key, req.ocr_lang)
    except ValueError as e:
        raise HTTPException(422, detail=str(e))
...
def settings_models():
    from app.aerbot.llm import ollama_inventory
    return ollama_inventory()


@app.get("/api/settings/models/pull-status")
def settings_pull_status():
    from app.aerbot.llm import pull_status
    return pull_status()


@app.post("/api/settings/models/pull", status_code=202)
def settings_models_pull(req: ModelPullRequest):
    from app.aerbot.llm import start_model_pull
    try:
        return start_model_pull(req.model)
    except ValueError as e:
        raise HTTPException(422, detail=str(e))
...
def factory_reset():
    """Golden rebuild + wipe uploads: pristine demo state."""
    import os
    import shutil
    from app.database import get_conn
    pipeline.reset_and_refresh()

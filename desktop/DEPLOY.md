...
in front, reachable 24/7. Cloud (Groq) mode keeps it cheap: ~1GB RAM, no
4.7GB model download. Offline mode needs ~8GB RAM instead.
...
- Revoke: delete the Groq key at console.groq.com/keys; app falls back to
  local model if RAM allows, else `/api/aerbot/*` returns 503 with guidance.
...
no Ollama/Tesseract binaries, 10–60s timeouts vs 20–130s answers). It needs a
